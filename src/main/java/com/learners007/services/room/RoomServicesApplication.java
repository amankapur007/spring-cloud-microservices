...
public class RoomServicesApplication {
	...
	@Bean
	public Docket api() {
		String basePackage = "com.learners007.services.room";
		return new Docket(DocumentationType.SWAGGER_2).groupName("Room").select()
				.apis(RequestHandlerSelectors.basePackage(basePackage)).paths(any()).build()
				.apiInfo(new ApiInfo("Room Services", "A set of services to provide data access to rooms", "1.0.0",
						null, new Contact("learners 007", "", null), null, null));
	}
